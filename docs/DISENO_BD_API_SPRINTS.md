...
> [PLAN_SPRINTS.md](PLAN_SPRINTS.md) (reparto por persona). Nada de lo que sigue está
...
## 1. Estado actual del proyecto (revisión 2026-09-17)
...
| S2 | **Conocimiento (F1)** | Subir audio → transcript → índice híbrido → chat con citas, corte vertical completo | E2E con clase sintética de 1 h; G1–G3 y avance G4/G5 |
| S3 | **Acción (F2)** | Extracción LLM, bandeja de revisión, Calendar idempotente, tools de lectura | G6/G7: confirmación → un único evento |
...
3. **§4** — los cuatro sprints desarrollados día a día por persona, conservando los
   identificadores de ticket de las fichas (`A1.3`, `S1.A6`, `J2.4`, `D3.2`, `L3.4`, …).
...
recall con datos representativos** (spec §6.2); los listados en §2.12 son candidatos.
...
erDiagram
    ...
    AUDIOS ||--o{ TASKS : deriva
    TASKS ||--o{ TASK_EVIDENCE : justifica
...
### 2.3 S1 — Identidad, sesión y materias (propietario: Andres)
...
- `sessions.token_hash` nunca guarda el valor plano de la cookie; revocación y expiración
  viven aquí (contrato de sesión congelado el lunes S1).
...
### 2.4 S1 esqueleto → S2 completo — Ingesta y audios (Andres: esquema; Santiago: pipeline)
...
CREATE TABLE audios (
  ...
  title                     text,
  teacher                   text,
  sha256                    text,                -- nullable hasta completar el upload
...
-- Identidad de dedupe exacta (spec M2): hash + idioma + materia + fecha + zona
...
-- Detección de variantes (mismo hash, contexto distinto) que exigen confirmación
...
CREATE TABLE ingestion_attempts (
  ...
  status            text NOT NULL
                      CHECK (status IN ('awaiting_upload','receiving','transcribing',
                                        'succeeded','requires_reupload','rejected','cancelled')),
  owner_instance    text NOT NULL,               -- propiedad del intento (una instancia)
  fencing_token     bigint NOT NULL,             -- un propietario vencido no puede publicar
  lease_expires_at  timestamptz,
  heartbeat_at      timestamptz,
  upload_expires_at timestamptz,                 -- TTL de inicio de upload (propuesta 10 min)
  receive_deadline  timestamptz,                 -- TTL de recepción (propuesta 30 min)
  asr_deadline      timestamptz,                 -- TTL ASR desde recepción (propuesta 60 min)
  ...
  cleanup_evidence  jsonb NOT NULL DEFAULT '{}', -- evidencia de cleanup, sin payload sensible
  audio_deleted_at  timestamptz,                 -- solo tras comprobar ausencia de temporales
  error_code        text,                        -- tipado, sin contenido sensible
  created_at        timestamptz NOT NULL DEFAULT now(),
  updated_at        timestamptz NOT NULL DEFAULT now()
);

-- Un único intento activo por clase; la concurrencia se resuelve transaccionalmente
...
Notas de diseño (spec §6.2):

- `audios` **no** tiene `storage_key`, blob ni ruta durable. El hash también se borra con la
  clase; los intentos fallidos sin transcript **reutilizan** el registro canónico en lugar de
  crear duplicados.
- Estado agregado (`processing|ready|partial|failed`) es **derivado en lectura** desde el
  intento y `processing_runs`; `not_requested` nunca equivale a éxito.
- `cleanup_status` está separado del ASR: lease expirado no prueba borrado; sin evidencia el
  estado queda `pending` y la outbox bloqueada. `cleanup_status='failed'` detiene la admisión
  local de nuevas ingestas y genera alerta.
- Los TTLs (10/30/60 min) son candidatos hasta el benchmark F0; los efectivos se publican en
  `GET /ingestion/capabilities`.

### 2.5 S2 — Transcript, chunks, embeddings e índice (Andres: esquema; José: lógica)
...
CREATE TABLE chunk_segments (
  ...
  FOREIGN KEY (user_id, segment_id) REFERENCES segments (user_id, id) ON DELETE CASCADE,
...
-- una sola generación activa por usuario; la activación es atómica (G5)
...
CREATE TABLE outbox_events (
  ...
  enabled          boolean NOT NULL DEFAULT false, -- gate: cleanup verificado
  status           text NOT NULL DEFAULT 'pending'
...
- El **shape** (nombre de campos y catálogo de `type`) se congela el lunes S1; Andres crea la
  tabla y los emisores; Jefferson implementa el dispatcher en S2 (regla anti-ciclo §5a).
- `enabled=true` solo ocurre tras `cleanup_status='verified'` del intento de ingesta origen.
- Los consumidores (`index`, `analyze`, `sync_task_event`, envío) son idempotentes y
  re-verifican propiedad/tombstone/versión antes de publicar.

### 2.7 S2 — Chat (Andres: esquema; José: pipeline)
...
- `message_sources` registra **todas** las fuentes entregadas al modelo (también las de
  ...
  fuente (G5). El texto ya visto por el usuario no puede retirarse; se informa ese límite.
- Historial con presupuesto de tokens y retención configurable por conversación.

### 2.8 S3 — Tareas, evidencia, revisiones y Calendar (Andres: esquema; Jefferson: sync)
...
CREATE TABLE tasks (
  ...
  version     integer NOT NULL DEFAULT 1,        -- locking optimista (PATCH)
...
CREATE INDEX task_evidence_by_task ON task_evidence (task_id, task_version);
...
CREATE TABLE task_revisions (
  ...
  task_id            uuid NOT NULL,
  FOREIGN KEY (user_id, task_id) REFERENCES tasks (user_id, id) ON DELETE CASCADE,
  run_id             uuid REFERENCES processing_runs(id),  -- origen si reanálisis
  revision_type      text NOT NULL CHECK (revision_type IN ('reanalysis','edit_of_confirmed')),
  proposal           jsonb NOT NULL,            -- campos propuestos; nunca muta la tarea viva
  evidence_validated boolean NOT NULL DEFAULT false,
  status             text NOT NULL DEFAULT 'proposed'
                       CHECK (status IN ('proposed','activated','discarded')),
  activated_at       timestamptz,
  created_at         timestamptz NOT NULL DEFAULT now()
);
...
- El ciclo de vida de la tarea (`status`) está **separado** de `calendar_sync_status`;
  `scheduled` no es un estado de tarea.
- Confirmar exige fecha válida + evidencia comprobada para tareas extraídas; los cambios de
  una tarea ya confirmada generan una `task_revisions` de tipo `edit_of_confirmed` que exige
  nueva confirmación explícita (un `PATCH` local nunca escribe Google).
- El borrado remoto solicitado vive en `google_remote_operations` y **sobrevive** a la purga
  de la tarea/clase hasta resolverse; no depende de FK con cascade.
...
### 2.9 S4 — Notificaciones y settings (Andres: esquema; Jefferson: scheduler)

```sql
CREATE TABLE settings (
  user_id                      uuid PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
  gmail_opt_in                 boolean NOT NULL DEFAULT false,   -- opt-in explícito (F3)
  digest_daily_enabled         boolean NOT NULL DEFAULT false,
  digest_daily_time            time NOT NULL DEFAULT '07:00',    -- hora local del usuario
  digest_weekly_enabled        boolean NOT NULL DEFAULT false,
  digest_weekday               integer NOT NULL DEFAULT 0,       -- 0 = domingo
  reminder_offsets_hours       jsonb NOT NULL DEFAULT '[48,24,2]',   -- instantes
  reminder_allday_offsets_days jsonb NOT NULL DEFAULT '[3,1]',       -- all-day, hora local
  calendar_write_calendar_id   text,             -- calendario secundario «Maulwurf»
  calendar_read_calendar_ids   text[] NOT NULL DEFAULT '{}',         -- lectura seleccionada
  created_at                   timestamptz NOT NULL DEFAULT now(),
  updated_at                   timestamptz NOT NULL DEFAULT now()
);
...
stateDiagram-v2
    ...
    state "Intento de ingesta (M2)" as A {
        ...
        transcribing --> succeeded: commit atómico + cleanup verified
        receiving --> requires_reupload: temporal perdido
        transcribing --> requires_reupload: deadline vencido
        awaiting_upload --> rejected: validación rechazada
    ...
    state "calendar_sync_status (M5)" as D {
        ...
        cs_pending --> cs_blocked: sin integración
    }
...
### 2.11 Matriz tabla → sprint → propietario

| Tabla | Sprint | Propietario | Consumidores |
|---|---|---|---|
| `users`, `sessions`, `google_credentials`, `subjects` | S1 | Andres | Todos |
| `audios`, `ingestion_attempts` | S1 esqueleto → S2 completo | Andres | Santiago (ingesta), Daniel (UI), Jefferson (jobs) |
| `outbox_events` | S1 shape → S2 dispatcher | Andres (tabla y emisores) | Jefferson (dispatcher y worker) |
| `transcripts`, `segments` | S2 | Andres (esquema) · Santiago (commit) | José (chunking/RAG), Daniel (lector) |
| `chunks`, `chunk_segments`, `embeddings`, `index_generations` | S2 | Andres (esquema) · José (lógica) | José (búsqueda), Daniel (citas) |
| `processing_runs` | S2 | Andres | José (`analyze`), Jefferson (runner), Santiago (backfill) |
| `conversations`, `messages`, `message_sources` | S2 | Andres | José (pipeline), Daniel (UI de chat) |
| `tasks`, `task_evidence`, `task_revisions`, `calendar_events`, `google_remote_operations` | S3 | Andres | Daniel (bandeja), Jefferson (sync), José (tools) |
| `settings`, `notifications` | S4 | Andres | Jefferson (scheduler), Daniel (panel) |
...
| CSRF | Toda mutación (`POST`/`PUT`/`PATCH`/`DELETE`, incluido el binario) exige `X-CSRF-Token` válido y `Origin` coincidente |
...
| `GET` | `/me` | S1 | Perfil + integraciones | `200` (shape abajo) |
...
| `DELETE` | `/me` | S4 | Borrado de cuenta: tombstone, revoca sesiones/jobs, purga derivados; si se pide, encola `google_remote_operations` **antes** de revocar tokens | `202` |
...
```json
{
  "audio_id": "8f0c…",
  "attempt_id": "c41b…",
  "upload_url": "/audios/8f0c…/content?attempt_id=c41b…",
  "upload_expires_at": "2026-09-17T23:15:00Z",
  "outcome": "new"
}
```
- `200 OK` (dedupe exacto por identidad de ingesta): `{"audio_id": "…", "outcome":
  "duplicate_exact"}` — clase canónica existente, sin nuevo ASR ni nueva URL.
- `409 duplicate_variant` (mismo hash, idioma/contexto distinto): envelope con
  `details.existing_audio_id`; el cliente debe confirmar la variante explícitamente y
  **volver a subir** (nunca se retiene audio esperando decisiones humanas). Una variante
  confirmada crea otra clase sin mutar evidencia ni tareas existentes.
- `422` metadatos inválidos / idioma fuera de allowlist / falta consentimiento.
- `429` cuota; `503` sin slots (la URL nunca se entrega sin reserva).
...
- `202 Accepted` tras recepción completa y admisión al proceso supervisado:
```json
...
- Errores: `410 upload_expired` · `413 payload_too_large` · `415 unsupported_format` ·
  `409 attempt_not_active` · `429`/`503`.
...
|---|---|---|---|---|
| `POST` | `/audios/{id}/retry` | S2 · S3 | Solo etapas sobre texto (`index`/`analyze`); crea `processing_runs` nuevos | `202` · `409` si no hay transcript utilizable |
...
| Commit atómico | Transacción única: `transcripts` + `segments` + `outbox_events` (`enabled=false`); idioma solicitado, modelo/configuración, versión y `timestamp_precision` persistidos; respuesta vacía = resultado sin texto utilizable, sin relleno |
| Cleanup | En `finally` al terminar ASR (éxito o error): cerrar buffers/archivos/cliente y eliminar original/convertidos/fragmentos; `cleanup_status` verificado independientemente antes de `succeeded` |
| Habilitación de outbox | Solo tras commit **y** cleanup verificado; el reconciliador post-commit verifica cleanup y habilita outbox sin retranscribir si la transacción ya se confirmó |
...
| `POST` | `/tasks` | Tarea manual o propuesta del chat (`source: manual|chat`); `confidence_score` nullable en manuales | `201` · `422` |
| `PATCH` | `/tasks/{id}` | Edición con locking optimista: el cuerpo debe traer `version`; desajuste → `409 version_conflict` con el estado vigente | `200` · `409` |
| `POST` | `/tasks/{id}/confirm` | Confirmación humana: guarda tarea + outbox de evento **atómicamente**; exige fecha válida y evidencia comprobada para extraídas | `202 {task, calendar_sync_status:"pending"}` · `422 date_unresolved|evidence_invalid` · si no hay integración: `202` con `calendar_sync_status:"blocked"` |
...
El mismo contrato alimenta el dashboard y la tool de chat `get_calendar_events(rango)`.

### 3.9 M6 — Búsqueda híbrida (S2; Ctrl+K en S4)

`GET /search?q=&subject_id=&from=&to=&limit=`
...
| `POST` | `/chat/conversations/{id}/messages` | S2 | Envía pregunta; **SSE consumido con `fetch`** (no `EventSource`, que solo permite GET); `client_message_id` idempotente | `200 text/event-stream` |
| `DELETE` | `/chat/conversations/{id}` | S2 | Tombstone de conversación | `204` |
| `POST` | `/chat/conversations/{id}/messages/{message_id}/cancel` | S2 (decisión) | Cancela el streaming en curso; el parcial queda `cancelled` | `202` |

Eventos SSE del chat (congelados S2; todos con `id`):
...
| `GET` | `/notifications?status=` | S4 (decisión) | Panel de entregas/sincronizaciones fallidas del dashboard |
| `POST` | `/notifications/{id}/resend` | S4 (decisión) | Reexpedición manual con advertencia explícita de posible duplicado |
...
| Semana | Contrato congelado | Proveedor | Consumidores |
|---|---|---|---|
| S1 | Sesión/cookie/CSRF (cookie `mw_session`, atributos, hash, revocación) | Andres | Todos |
| S1 | `POST /audios` + `PUT` binario (URL relativa, expiración, `attempt_id`) | Andres | Santiago, Daniel |
| S1 | Shape de `outbox_events` (campos + catálogo de `type`) | Andres | Jefferson |
| S1 | Estados de ingesta (`awaiting_upload→…`) y su exposición SSE | Santiago/Andres | Daniel |
| S2 | Shape de chunks/embeddings y eventos de `index(audio_id)` | José/Andres | Jefferson |
| S2 | Eventos SSE del chat (`delta|citation|done|error`) y formato de citas | José | Daniel |
| S2 | Contrato de `/search` para Ctrl+K | José | Daniel |
| S3 | Contrato `tasks` + revisiones (`task_revisions`) y `calendar_events` | Andres | Daniel, Jefferson |
| S3 | Tools de chat de lectura y sus límites de iteración | José | Jefferson, Daniel |
| S4 | Preferencias/notificaciones (`settings`, `notifications`, dedupe) | Andres | Jefferson, Daniel |
...
| `PATCH /me/notifications`, `POST /notifications/test`, `DELETE /me` | — | — | — | ✓ |
...
el trabajo de otro); viernes = demo + puerta de salida + retro. Nada se mergea sin 1 review;
el código que cruza fronteras lo revisa el dueño de la carpeta. Definición de hecho por
ticket: código + tests + documento/contrato actualizado + demostrable en compose local.
...
#### José Leonardo (líder · RAG)

| Día | Ticket | Tarea | Entregable verificable |
|---|---|---|---|
| Lunes | L1.6 | Congelar interfaces S1 en el plan maestro | Tabla §5 del plan sin cambios toda la semana |
| 1 | L1.1 | Crear estructura monorepo §3.3 y repos; fijar versiones (Python, Node, Postgres 16, pgvector) y lockfiles | `apps/` + `infra/` + lockfiles en el repo |
| 1–2 | L1.2 | ADRs cortos: estructura, versiones, política de lockfiles, formato de commit/PR | ADRs en `docs/` revisados el viernes |
| 2 | L1.3 | Protocolo del spike de Santiago: qué medir, formato del informe, criterios D2/D4/D6 | Protocolo publicado antes del día 2 y usado como único formato |
...
| Continuo | L1.7 | Revisar todos los PRs; mantener tablero | PRs con review del dueño |
...
| Día | Ticket | Tarea | Entregable verificable |
|---|---|---|---|
| 1 | A1.1 | Scaffold `apps/api`: FastAPI + Pydantic v2 + SQLAlchemy 2 async (routers/services/models/core) | API arranca en compose |
| 1 | A1.2 | Config con Pydantic Settings; `NVIDIA_API_KEY` y secretos solo por entorno | Secretos fuera de código y logs |
...
Alcance: el `PUT` binario S1 es **solo esqueleto**; la recepción real la completa Santiago en
S2 sobre el contrato congelado. Plan B si las credenciales OAuth no llegan el día 2: sesión
opaca + CSRF con login de desarrollo tras feature flag temporal, nunca mergeado a demo.

#### Santiago (audio · transcripción) — semana crítica, spike bloqueante
...
| Viernes | S1.A9 | Presentar informe; con José y el equipo cerrar D2/D4/D6 | Decisiones cerradas o bloqueadas con propuesta |
...
#### Jefferson (integraciones · infra · calidad)
...
| 2 | J1.8 | Imagen base con ffmpeg para el spike de Santiago (sin endurecer aún) | Spike desbloqueado día 2 |
...
| 4 | D1.6 | CRUD materias contra la API de Andres | Materias creables/editables |
| 5 | D1.7 | Borradores estáticos de upload y biblioteca (sin wiring) para validar UX temprano | Revisión del equipo el viernes |

Plan B: si los endpoints de Andres se retrasan, mock server contra el contrato congelado y
...
citas válidas; G1–G3 pasan; G4/G5 con avance medible.
...
| 4 | L2.4 | `SearchService` reutilizable + `GET /search` (con Andres) | Endpoint con snippet+audio+timestamp |
...
| 5 | A2.11 | `GET /search` (con José): filtros tenant/materia/fechas; snippet + audio + timestamp válido | Contrato Ctrl+K |

#### Santiago (pipeline de ingesta real)
...
| 4 | S2.5 | Commit atómico transcript+segmentos (con Andres): idioma, modelo/config, versión, `timestamp_precision`; vacío/silencio = sin texto utilizable; calidad baja → advertencias | Transacción coherente |
...
#### Jefferson (workers · observabilidad)
...
| 2–3 | J2.2 | Job `index(audio_id)` idempotente: consume la lógica de José; verifica propiedad/tombstone/versión antes de publicar; reintentos backoff+jitter (máx 3 transitorios) | Reintentos sin duplicados |
...
| 5 | J2.7 | Fault injection con Andres/Santiago: crash antes/después del commit; lease vencido no marca cleanup verificado | Evidencia documentada |
...
G1/G5 con evidencia (crash antes/después de commit resuelto; sin versiones parciales ni
...
con cuenta Google de prueba; G6/G7 aprobadas.
...
#### José Leonardo (prompts · validación · fechas)
...
| 4–5 | L3.6 | Job `analyze(audio_id)` (con Jefferson en `workers/`); reanálisis crea revisión nueva sin sobrescribir tareas editadas/confirmadas | Backfill explícito |
...
#### Andres (tareas · evidencia · revisiones)
...
| 3–4 | A3.6 | Reconstrucción de evidencia desde BD para validar spans de José: offsets Unicode por segmento; rechazar pertenencia inválida | Ninguna propuesta con span inventado |
...
| 4–5 | A3.8 | Tabla `calendar_events` espejo (clave `(user_id, calendar_id, event_id)`, all-day separado de instantes, ETag, `last_synced_at`) | Lista para el worker de Jefferson |
...
biblioteca/chat; G6/G7 con evidencia.
...
#### José Leonardo (cierre de calidad)
...
| Continuo | L4.4 | Si cambió algún contrato: spec + plan actualizados en la misma revisión | Consistencia |
| 4 | L4.5 | Go/no-go de demo y guion (upload → transcript → chat con cita → tarea → confirmación → evento → recordatorio) | Guion ensayado el jueves con Daniel |
| Viernes | L4.6 | Retro final y backlog F4 priorizado | Propuesta priorizada |

#### Andres (preferencias · borrado · cierre)
...
| 2–3 | A4.4 | Borrado de cuenta: tombstone, bloquear sesiones/jobs, purgar datos y fuentes derivadas; si se piden eventos, ejecutar borrado remoto **antes** de revocar tokens | E2E de borrado |
| 3 | A4.5 | Política D8: backups solo texto/metadatos cifrados, retención 30 días, borrados reaplicados al restaurar | Restore comprobado |
...
#### Santiago (hardening de ingesta)

| Día | Ticket | Tarea | Entregable verificable |
|---|---|---|---|
| 1–2 | S4.1 | Runbook de cleanup/recuperación: cleanup fallido bloquea admisión, sweeper cada 60 s, pérdida de host, restauración | Runbook revisado por Jefferson |
...
#### Jefferson (Gmail · scheduler · despliegue)
...
#### Daniel (dashboard final · Ctrl+K · demo)
...
| 4 | D4.7 | Ensayo de demo con José: guion completo | Dos ejecuciones (jueves y viernes) |
| 5 | D4.8 | Revisión final de privacidad del navegador con José (checklist G1 en cliente) | Checklist verificada |
...
| G2 ASR | Idioma respetado; formatos/límites probados; offsets ordenados y en rango; cero tiempos inventados | spike y decisión D6 | contrato con transcripts reales | — | — |
...
| G5 Recuperación | Sin versiones parciales ni duplicados; crash resuelto según contrato | — | fault injection + reconciliador | — | — |
| G6 Extracción | ≥50 actividades/idioma; precisión ≥ 0.95; recall ≥ 0.85; fecha exacta ≥ 0.95; evidencia 100% | — | — | medición | cierre |
| G7 Google | Nada sin confirmación; evento único ante retries; all-day/instantes y scopes comprobados; entrega incierta sin reintento | — | — | Calendar | Gmail |
| G8 Operación | Builds/tests 3 apps; restore; recordatorios ≤ 15 min; presupuesto/alertas | CI base | smoke E2E | — | cierre |

### 4.6 Resumen anti-ciclo (regla §5a del plan maestro)

Quien provee una interfaz la congela el lunes y entrega el código a más tardar el día 3;
quien consume construye contra el contrato congelado (con mock si hace falta) y conmuta a
código real el miércoles/jueves. Nadie espera bloqueado más de un día.

| Persona | No espera a | Consume desde el lunes | Código real llega |
|---|---|---|---|
| Andres | Jefferson (compose) ni José (versiones) — S1; Santiago (Riva) — S2 | Decisiones de versiones (día 1); shape de commit congelado | Compose día 1–2; commit simulado con fixtures en S2 |
| Santiago | Jefferson (imagen endurecida) ni Andres (endpoints) | Contrato `POST /audios`+`PUT` congelado | Imagen base día 2; endpoints día 2–3 |
| José | Santiago (spike/transcripts) ni Andres (tablas) | Sus propios contratos (él publica) | Tablas día 2; transcripts reales solo para medir el viernes |
| Daniel | Andres (endpoints) ni Jefferson (compose/web) | Contratos de sesión/materias/estados congelados | API real día 3–4; conmuta jueves |
| Jefferson | Andrés (esqueleto API) y José (índice real) | ADR de versiones (día 1); contrato del job índice congelado | Índice stub → real de José día 4–5 |
...
- [PLAN_SPRINTS.md](PLAN_SPRINTS.md) — plan maestro de 4 semanas y reglas de coordinación.
- [PLAN_TESTS.md](PLAN_TESTS.md) — catálogo de pruebas por sprint y persona.
- [NVIDIA_RIVA.md](NVIDIA_RIVA.md) — parámetros auditados para el spike S1.
- Fichas: [sprints/](sprints/README.md) — checklists personales con evidencia.
